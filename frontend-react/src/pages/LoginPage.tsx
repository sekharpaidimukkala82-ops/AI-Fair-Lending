...
function BackendStatus() {
  ...
  const color = status === 'online' ? 'bg-green-400' : status === 'offline' ? 'bg-red-500' : 'bg-yellow-400'
  const pulse = status === 'online' ? 'animate-pulse' : ''
  const title = status === 'online' ? 'API Online' : status === 'offline' ? 'API Offline' : 'Checking API...'

  return (
    <div className="fixed top-4 right-4 z-50" title={title}>
      <span className={`block w-3 h-3 rounded-full ${color} ${pulse}`} />
    </div>
  )
}
